(function() {
    ...
    */
    ...
    function hasClass(markup, cssClass) {
        var regex = 'class=["\'](.* )?' + cssClass + '( .*)?["\']'
        return !!markup.match(regex, 'gm');
    }

    var classes = ['container', 'text', 'date', 'excerpt', 'main', 'lorem'];

    classes.forEach(function(index){
        console.log(hasClass(markup, index) + ' para a classe ' + index);
    });
}());
